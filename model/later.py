...
class LaterModel(Query):
    def __init__(self, db):
        self.db = db
        self.table_name = "later"
        super(LaterModel, self).__init__()

    def add_new_later(self, later_info):
        return self.data(later_info).add()

    def delete_later_info_by_user_id_and_post_id(self, user_id, post_id):
        where = "user_id = %s AND post_id = %s" % (user_id, post_id)
        return self.where(where).delete()

    def delete_later_by_post_id(self, post_id):
    ...
    def get_user_later_count(self, user_id):
        where = "user_id = %s" % user_id
    ...
    def get_user_all_laters(self, user_id, num = 3, current_page = 1):
        ...
        order = "later.created DESC, later.id DESC"
        field = "post.*, \
    ...
    def get_user_all_later_posts_by_nav_id(self, user_id, nav_id, num = 16, current_page = 1):
        where = "later.user_id = %s AND '%s' = later.nav_id" % (user_id, nav_id)
        join = "LEFT JOIN post ON later.post_id = post.id\
                LEFT JOIN user AS author_user ON post.author_id = author_user.uid \
                LEFT JOIN channel ON post.channel_id = channel.id \
                LEFT JOIN video ON post.video_id = video.id \
